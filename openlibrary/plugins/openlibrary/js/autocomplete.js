 ...
 *      <a class="mia__to_top" href="javascript:;">[x]</a>
 *      <a class="mia__to_bottom" href="javascript:;">[x]</a>
...
export function setupMultiInputAutocomplete($) {    
    $.fn.setup_multi_input_autocomplete = function(input_renderer, ol_ac_opts, ac_opts) {
        ...
        function update_indices() {
            container.find('.mia__input').each(function(index) {
                $(this).find('.mia__index').text(`≡ #${index + 1}`);
        ...
        function updateButtonVisibility() {
            const items = container.find('.mia__input');
            items.each(function(index) {
                const isFirst = index === 0;
                const isLast = index === items.length - 1;
                $(this).find('.mia__to_top').toggle(!isFirst);
                $(this).find('.mia__to_bottom').toggle(!isLast);
            });
        }
        ...
        container.on('click', '.mia__to_top', function() {
            var item = $(this).closest('.mia__input');
            item.prependTo(container);
            update_indices();
        });
    
        container.on('click', '.mia__to_bottom', function() {
            var item = $(this).closest('.mia__input');
        ...
        container.on('click', '.mia__add', function(event) {
            ...
            var next_index = container.find('.mia__input').length;
